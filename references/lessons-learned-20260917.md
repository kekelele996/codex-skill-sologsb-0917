...
- 2026-09-22 修订：每个容器在 `docker run` 前都必须在主机级独占锁内取槽；准入计数必须同时包含运行中容器和存活预占位。只统计运行中容器会在 `docker run` 到 `docker ps` 的时间差里被多个执行器同时突破上限。上限动态读取设备配置 `~/.codex/sologsb/config.json` 的 `claude.maxContainers`，默认 4，绝对硬顶 6（2026-09-26 起为 8，弹性扩容可能把上限调到 6 以上，属正常）；排队等待时每 5 秒重读一次（2026-09-26 起，并按领号先后放行）。
...
- 固定处理顺序：发布阶段先在 `.git/info/exclude` 排除依赖目录、构建产物和缓存；stage 后撤回未配对的锁文件（依赖清单改了就随清单发布，见 `change-volume-gate.md`），再
