...
- 2026-09-22 修订：每个容器在 `docker run` 前都必须在主机级独占锁内取槽；准入计数必须同时包含运行中容器和存活预占位。只统计运行中容器会在 `docker run` 到 `docker ps` 的时间差里被多个执行器同时突破上限。上限动态读取设备配置 `~/.codex/sologsb/config.json` 的 `claude.maxContainers`，默认 4，绝对硬顶 6；排队等待时每 180 秒重读一次。
