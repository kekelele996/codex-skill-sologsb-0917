...
每个逻辑侧只能提交一个有效轨迹文件：`workspace/轨迹文件/a` 与 `workspace/轨迹文件/b` 的顶层必须各自恰好包含一个 `.jsonl`。失败尝试可以保留在 `rejected/` 子目录，但不能放在顶层或上传到平台；生成 Excel、状态复核、预检和上传接口都会复验数量，0 个或超过 1 个均阻断。
